...
### 시나리오 예시 — 왼쪽으로 걸어가던 사람

실제 데이터에서 뽑은 한 사례 (`cam_1` 영상, `id 11`, 31번 프레임부터):

![scenario](assets/scenario.png)

```
① 관찰   8프레임 동안 x가 476 → 448로 이동 (왼쪽으로 걷는 중)
         추정 속도 = 프레임당 왼쪽 4.0px, 아래 0.7px
② 예측   "이 속도로 계속 가면 8프레임 뒤엔 (416, 431)에 있겠다"
③ 실제   실제로 간 곳은 (402, 436)
   결과   76px 이동했고, 예측은 15px 빗나감
```

이 사람이 **왼쪽으로 가려던 의도**를 8프레임 앞서 맞힌 것이다. 로봇이 왼쪽에 있었다면 이 시점에 이미 감속을 시작할 수 있다. 반대로 오른쪽에 있었다면 **가까이 있어도 멈추지 않는다** — 멀어지는 중이기 때문.
