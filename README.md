...
## 공개 데이터(ETH) 벤치마크 — 등속 vs 칼만 vs LSTM

실제 보행자 궤적(ETH, OpenTraj)에서 표준 프로토콜(관찰 8스텝/3.2s → 예측 12스텝/4.8s)로 비교. 실행: `python -m trajectory.eth_eval` (ETH 데이터 자동 다운로드, 작은 LSTM 즉석 학습).

| | ADE (m) | FDE (m) |
|---|---|---|
| **교차 장면** (hotel 학습 → eth 평가, 분포 다름) | | |
| 등속 | 0.577 | 1.163 |
| 칼만 | 0.552 | 1.123 |
| LSTM | **2.393** ❌ | **4.610** ❌ |
| **동일 분포** (eth+hotel 80/20 혼합) | | |
| 등속 | 0.480 | 0.970 |
| 칼만 | 0.456 | 0.932 |
| LSTM | **0.370** ✅ | **0.717** ✅ |

**결론**: 학습형(LSTM)은 **배포 환경과 같은 분포의 데이터가 있을 때만** 이득(ADE 0.37 vs 0.46). 분포가 다르면 오히려 **크게 붕괴**(2.39). 칼만은 분포가 바뀌어도 강건. → 실제 조리실 궤적 데이터가 쌓이기 전엔 **칼만이 안전한 기본값, LSTM은 보류**가 데이터로 뒷받침된다.

> LSTM은 벤치마크용으로 즉석 학습·평가만 하고 저장하지 않는다. 배포용 학습형 모델은 실제 조리실 궤적 데이터 확보 후 학습·저장한다.

## 천장뷰 도메인 벤치마크 — YOLO 오버헤드 데이터셋에서 궤적 생성

ETH는 거리·군중 도메인이라, **우리가 실제 쓰는 천장뷰**로도 검증했다. YOLO 학습에 쓴 오버헤드 데이터셋(24개 연속영상 클립)의 GT 박스를 **프레임 간 IoU로 추적해 ID를 부여**하고 (x,y) 궤적을 만들었다. 실행: `python -m trajectory.overhead_lstm`. (23클립 · 429 트랙 · 33,633 윈도우, 관찰 8프레임 → 예측 8프레임, ADE/FDE는 640px 환산.)
...
**결론 (ETH와 동일 패턴 재확인)**: 천장뷰 도메인에서도 — 같은 분포 데이터가 있으면 **LSTM이 이득**(ADE 7.1 vs 8.3), 분포가 다르면 **등속·칼만이 더 강건**(LSTM 6.8 > 등속 6.2). 또한 오차가 프레임의 1~2%로 작아, **천장뷰 사람은 느리고 예측이 쉬워 등속/칼만만으로도 충분**하다. 학습형은 "충분한 동일 도메인 궤적 데이터"가 있을 때만 의미가 있다.

## 실제 조리 CCTV 검증에서 드러난 Phase 1 한계

실제 조리 로봇 CCTV 영상에 검출 모델을 돌려본 결과(별도 확인):
- **위생모(캡)는 검출을 막지 않았다** — 잘 보이는 작업자는 stock YOLO가 잘 검출.
- 그러나 **천장뷰로 파인튠한 모델은 이 각진 CCTV에서 실패**(실제 사람 미탐 + 설비 오탐). 원인은 **카메라 각도 불일치**(학습=탑다운, 실제=측면 각도).
- **은색 천 커버 로봇팔이 사람으로 오탐**되는 것이 이 환경의 실제 난제.

→ 실제 배포 전 **실제 카메라 화각의 조리실 영상으로 재학습**(또는 카메라를 진짜 천장 수직 설치)이 필요. 검출 모델: [chanubc/overhead-person-yolo11](https://github.com/chanubc/overhead-person-yolo11)
